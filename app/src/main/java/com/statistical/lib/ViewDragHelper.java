...
public class ViewDragHelper {
    ...
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                Log.e("mDragState","mDragState==="+mDragState);
                if (mDragState == STATE_DRAGGING) {
    ...
    public boolean isEdgeTouched(int edges, int pointerId) {
        ...
        Log.e("tryCaptureView","tryCaptureView==="+isPointerDown(pointerId)+"   "+((mInitialEdgeTouched[pointerId] & edges) != 0));
    ...
    private int getEdgeTouched(int x, int y) {
        int result = 0;
       // Log.e("  resultX","resultX==="+x+"     LEFT=" +mParentView.getLeft()+"     EDGESIZE=" + mEdgeSize);
        ...
        else if (x > mParentView.getRight() - mEdgeSize) {
            result = EDGE_RIGHT;
        }
        else if (y > mParentView.getBottom() - mEdgeSize) {
            result = EDGE_BOTTOM;
        }
        return result;
    }

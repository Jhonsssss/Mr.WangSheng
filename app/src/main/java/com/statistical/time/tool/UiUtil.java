...
public class UiUtil {
    ...
    public static Context getContext() {
        return MyApplication.getLanguageContext();
    }
...
//            DecimalFormat decimalFormat = new DecimalFormat("0.00");//构造方法的字符格式这里如果小数不足2位,会以0补足.
     ...
     * 属性动画，周边空间进入屏幕
     */
    public static void aroundLayoutIn(View left, View right, View bottom) {
        if(null != left){
            ValueAnimUtils.leftAnimator(-left.getWidth(), 0, left, 0, 450);
        }
        if(null != right){
            ValueAnimUtils.rigthAnimator(-right.getWidth(), 0, right, 0, 450);
        }
        if(null != bottom){
            ValueAnimUtils.bottomAnimator(-bottom.getHeight(), 0, bottom, 0, 450);
        }
    }
     ...
     * 属性动画，周边空间离开屏幕
     */
    public static void aroundLayoutExit(View left, View right, View bottom) {
        if(null != left){
            ValueAnimUtils.leftAnimator(0, -left.getWidth(), left, 0, 450);
        }
        if(null != right){
            ValueAnimUtils.rigthAnimator(0, -right.getWidth(), right, 0, 450);
        }
        if(null != bottom){
            ValueAnimUtils.bottomAnimator(0, -bottom.getHeight(), bottom, 0, 450);
        }
    }
